...
main (int argc, char *argv[])
{
  CommandLine cmd;
  std::string protocol;
  cmd.AddValue("protocol", "Name of Protocol", protocol);
  cmd.Parse (argc, argv);
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream (protocol.substr(5)+"part1.cwnd");
  ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CwndChange, stream));

  PcapHelper pcapHelper;
  Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("part1.pcap", std::ios::out, PcapHelper::DLT_PPP);
